...
function loadConversation(root) {
  const file = conversationPath(root);
  if (!fs.existsSync(file)) return [];
  try {
    const parsed = JSON.parse(fs.readFileSync(file, 'utf8'));
    return Array.isArray(parsed.exchanges) ? parsed.exchanges.filter(item => item && typeof item.user === 'string' && typeof item.assistant === 'string') : [];
  } catch { return []; }
}

function saveConversationExchange(root, user, assistant) {
  const exchanges = loadConversation(root);
  exchanges.push({ at: new Date().toISOString(), user: String(user).slice(0, 4000), assistant: String(assistant).slice(0, 8000) });
  while (exchanges.length > 12) exchanges.shift();
  while (JSON.stringify(exchanges).length > 30000 && exchanges.length > 1) exchanges.shift();
  const file = conversationPath(root);
