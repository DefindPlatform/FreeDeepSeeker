...
function sanitizeEnvironment(env = process.env) {
  ...
  for (const [key, value] of Object.entries(env)) {
    if (/(TOKEN|SECRET|PASSWORD|COOKIE|AUTH|CREDENTIAL|PRIVATE_KEY)/i.test(key)) continue;
    clean[key] = value;
  }
  return clean;
}
...
function validateCommand(program, args, root, config) {
  const executable = path.basename(String(program || '')).replace(/\.(exe|cmd|bat)$/i, '').toLowerCase();
...
function runProgram(program, args, options) {
  ...
  return new Promise(resolve => {
    ...
    let stderr = '';
    let timedOut = false;
    const append = (current, chunk) => Buffer.byteLength(current) >= maxOutputBytes ? current : (current + chunk.toString()).slice(-maxOutputBytes);
