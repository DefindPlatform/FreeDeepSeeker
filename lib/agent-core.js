...
function workspaceSessionId(root) {
  const realRoot = fs.realpathSync(root);
  const identity = process.platform === 'win32' ? realRoot.toLowerCase() : realRoot;
  return `coding-agent-${crypto.createHash('sha256').update(identity).digest('hex').slice(0, 20)}`;
}

function conversationPath(root) {
  return path.join(fs.realpathSync(root), STATE_DIR, CONVERSATION_FILE);
}

function loadConversation(root, config = {}) {
  if (config.historyEnabled === false) return [];
  const file = conversationPath(root);
  if (!fs.existsSync(file)) return [];
  try {
    const parsed = JSON.parse(fs.readFileSync(file, 'utf8'));
    const ttlDays = Math.min(Math.max(Number(config.historyTtlDays) || 30, 1), 365);
    const cutoff = Date.now() - ttlDays * 86400000;
    return Array.isArray(parsed.exchanges) ? parsed.exchanges.filter(item => item && typeof item.user === 'string' && typeof item.assistant === 'string' && (!item.at || Date.parse(item.at) >= cutoff)) : [];
  } catch { return []; }
}

function saveConversationExchange(root, user, assistant, config = {}) {
  if (config.historyEnabled === false) return [];
  const exchanges = loadConversation(root, config);
  exchanges.push({ at: new Date().toISOString(), user: String(user).slice(0, 4000), assistant: String(assistant).slice(0, 8000) });
  const maxExchanges = Math.min(Math.max(Number(config.maxConversationExchanges) || 12, 1), 50);
  const maxChars = Math.min(Math.max(Number(config.maxConversationChars) || 30000, 1000), 200000);
  while (exchanges.length > maxExchanges) exchanges.shift();
  while (JSON.stringify(exchanges).length > maxChars && exchanges.length > 1) exchanges.shift();
  const file = conversationPath(root);
  fs.mkdirSync(path.dirname(file), { recursive: true });
  atomicWrite(file, `${JSON.stringify({ version: 1, exchanges }, null, 2)}\n`);
  return exchanges;
}

function contextTerms(value) {
  return new Set(String(value || '').toLowerCase().match(/[a-zа-яё0-9_.-]{3,}/gi) || []);
}

function selectConversationContext(exchanges, task, maxChars = 30000) {
  const limit = Math.min(Math.max(Number(maxChars) || 30000, 1), 200000);
  const wanted = contextTerms(task);
  const ranked = exchanges.map((exchange, index) => {
    const terms = contextTerms(`${exchange.user}\n${exchange.assistant}`);
    let overlap = 0;
    wanted.forEach(term => { if (terms.has(term)) overlap += 1; });
    return { exchange, index, score: overlap * 1000 + index };
  }).sort((a, b) => b.score - a.score);
  const selected = [];
  let chars = 0;
  for (const item of ranked) {
    const size = item.exchange.user.length + item.exchange.assistant.length;
    if (chars + size > limit) continue;
    selected.push(item);
    chars += size;
  }
  return selected.sort((a, b) => a.index - b.index).map(item => item.exchange);
}

function clearConversation(root) {
  const file = conversationPath(root);
...
function loadProjectConfig(root) {
  ...
  config.maxRunDurationMs = Math.min(Math.max(Number(config.maxRunDurationMs) || defaults.maxRunDurationMs, 1000), 86400000);
  if (!['process', 'docker'].includes(config.commandSandbox)) throw new Error('commandSandbox должен быть process или docker');
  if (!/^[a-z0-9][a-z0-9._/@:-]{0,199}$/i.test(String(config.dockerImage))) throw new Error('Некорректный dockerImage');
  config.sandboxMemoryMb = Math.min(Math.max(Number(config.sandboxMemoryMb) || defaults.sandboxMemoryMb, 128), 8192);
  config.sandboxCpu = Math.min(Math.max(Number(config.sandboxCpu) || defaults.sandboxCpu, 0.25), 8);
  config.sandboxNetwork = config.sandboxNetwork === true;
  config.historyEnabled = config.historyEnabled !== false;
  config.historyTtlDays = Math.min(Math.max(Number(config.historyTtlDays) || defaults.historyTtlDays, 1), 365);
  config.maxConversationExchanges = Math.min(Math.max(Number(config.maxConversationExchanges) || defaults.maxConversationExchanges, 1), 50);
  config.maxConversationChars = Math.min(Math.max(Number(config.maxConversationChars) || defaults.maxConversationChars, 1000), 200000);
  return config;
}
...
class RunTransaction {
  ...
  rollback(status = 'undone') {
    return restoreRun(this.root, this.manifestPath, status);
  }
}
...
function undoLatestRun(root) {
  ...
  return restoreRun(root, manifestPath, 'undone');
}

function restoreRun(root, manifestPath, status) {
...
function buildCommandInvocation(program, args, options) {
  if (options.sandbox !== 'docker') return { executable: program, args };
  return {
    executable: 'docker',
    args: [
      'run', '--rm', '--init',
      '--network', options.sandboxNetwork ? 'bridge' : 'none',
      '--cpus', String(options.sandboxCpu),
      '--memory', `${options.sandboxMemoryMb}m`,
      '--pids-limit', '128', '--cap-drop', 'ALL', '--security-opt', 'no-new-privileges',
      '-v', `${options.cwd}:/workspace`, '-w', '/workspace',
      options.dockerImage, program, ...args,
    ],
  };
}
...
function runProgram(program, args, options) {
  ...
  return new Promise(resolve => {
    const invocation = buildCommandInvocation(program, args, options);
    let executable = invocation.executable;
    let actualArgs = invocation.args;
    const normalized = path.basename(String(program)).replace(/\.(cmd|bat|exe)$/i, '').toLowerCase();
    if (options.sandbox !== 'docker' && process.platform === 'win32' && ['npm', 'npx'].includes(normalized)) {
      ...
      actualArgs = [cli, ...args];
    } else if (options.sandbox !== 'docker' && process.platform === 'win32' && /\.(cmd|bat)$/i.test(String(program))) {
      resolve({ exit_code: null, error: 'Запуск .cmd/.bat без shell не поддерживается; используйте прямой executable', stdout: '', stderr: '' });
      return;
    }
    let child;
    try { child = spawn(executable, actualArgs, { cwd, shell: false, windowsHide: true, env, detached: process.platform !== 'win32' }); }
    catch (error) {
...
module.exports = {
  ...
  RunTransaction, undoLatestRun, sanitizeEnvironment, validateCommand, buildCommandInvocation, terminateProcessTree, runProgram,
  workspaceSessionId, loadConversation, saveConversationExchange, selectConversationContext, clearConversation,
};
