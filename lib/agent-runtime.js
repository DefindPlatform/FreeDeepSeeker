...
class AgentRunController {
  ...
  remainingTimeMs() {
    if (this.startedAtMs === null) return this.maxDurationMs;
    return Math.max(0, this.maxDurationMs - (this.clock() - this.startedAtMs));
  }

  createDeadlineSignal() {
    this.assertActive();
    return AbortSignal.timeout(Math.max(1, this.remainingTimeMs()));
  }
