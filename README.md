...
Запустите CLI из папки проекта:

```bash
npm run client
```

Для запуска из любой папки один раз зарегистрируйте глобальную команду:
...
CLI покажет пронумерованный список моделей с возможностями reasoning/search/files. Внутри чата модель можно сменить командой `/model`, начать чистый разговор через `/new`, настроить системную инструкцию через `/system` и посмотреть остальные команды через `/help`.

Если локальный proxy ещё не работает, CLI автоматически запустит его в фоне. Для отключения этого поведения используйте `--no-auto-start`.

Одиночный запрос без интерактивного режима:

```bash
npm run client -- -m deepseek-reasoner "Объясни квантовую запутанность кратко"
```

Другой адрес proxy задаётся параметром `--url` или переменной `DEEPSEEK_API_URL`.

### Coding-agent с доступом к проекту

Команда `deepseek-agent` даёт модели инструменты для чтения и поиска файлов, создания и точечного изменения кода, удаления путей и запуска тестов/команд. Рабочая область всегда ограничена выбранной папкой.
...
Запуск прямо из домашней папки пользователя блокируется, чтобы агент случайно не получил доступ ко всему профилю. Перейдите в каталог проекта или передайте его через `-C`. Осознанно снять защиту можно только флагом `--allow-home`.

Посмотреть полный индекс проекта без запуска модели:

```bash
deepseek-agent -C C:\\path\\to\\project --project-map
deepseek-agent -C C:\\path\\to\\project --project-map --json
```

В интерактивном режиме `/project` показывает карту, а `/project src` фильтрует её. Перед каждой задачей агент автоматически получает языки, тесты, ключевые конфигурации и карту файлов; для больших репозиториев полный индекс доступен модели постранично через `get_project_map`.

Разовая задача:

```bash
deepseek-agent "Изучи проект, исправь падающие тесты и проверь результат"
```

По умолчанию запись, удаление и запуск команд требуют подтверждения. Полностью автономный режим включается явно:

```bash
deepseek-agent --yes "Добавь в проект REST endpoint и тесты"
```

Другую рабочую папку можно передать без `cd`: `deepseek-agent -C C:\\path\\to\\project "задача"`. Флаг `--yes` отключает подтверждения для разрешённых политикой операций, поэтому используйте его только в папках, которым доверяете.

Coding-agent использует три режима разрешений:

- `read-only` — только анализ проекта;
- `ask` — показывает сокращённый diff и спрашивает перед мутациями/командами;
- `full` — автономная работа в рамках локальной политики.

```bash
deepseek-agent --mode read-only "Проведи ревью архитектуры"
deepseek-agent --mode ask "Исправь обработку ошибок"
deepseek-agent --mode full "Обнови зависимости и тесты"
```

Все изменения журналируются в `.deepseek-agent/runs/`, исходные версии файлов сохраняются перед первой мутацией. Последний завершённый запуск можно безопасно откатить; если файл после запуска изменялся вручную, агент остановит откат вместо перезаписи:
...
```json
{
  ...
  "allowedPrograms": ["node", "npm", "git", "python", "pytest"],
  "commandTimeoutMs": 30000,
  "rollbackOnFailure": true
}
```

`.env`, приватные ключи, credential-файлы и `deepseek-auth.json` закрыты от модели по умолчанию. Команды запускаются напрямую без shell, цепочек, pipe и перенаправлений; изменяющие Git-команды агенту запрещены.

### Agent Studio — визуальная работа с проектом
