...
По умолчанию новые задачи в одной рабочей папке продолжают предыдущий проектный диалог. История хранится локально в `.deepseek-agent/conversation.json`; в интерактивном режиме команда `/new` начинает новый диалог. Для разового запуска без чтения и сохранения контекста используйте `--no-history`, а для постоянного приватного режима установите `historyEnabled: false` в `.deepseek-agent.json`.
