...
- `get_task_plan`, `set_task_plan` and `update_task` — dependency-aware durable planning and progress
...
`--max-steps` limits model round trips, `--max-tool-calls` independently limits local tool requests, and `--max-duration-ms` overrides the configured wall-clock run budget. The runtime rejects a fourth identical tool call by default, including calls whose JSON object keys only differ in order. This prevents a confused model from consuming the complete budget in a tight loop.
...
```powershell
deepseek-agent --dry-run --report .deepseek-agent\report.json "Обнови API"
deepseek-agent --max-steps 40 --max-tool-calls 160 --max-duration-ms 1800000 "Исправь тесты"
```
...
The current dependency-aware work plan is stored in `.deepseek-agent/plan.json`. It is capped, cycle-checked, revisioned for concurrent-update protection, and included in transaction rollback. The model can create a plan, inspect ready work and progress, and update tasks only through legal state transitions.
